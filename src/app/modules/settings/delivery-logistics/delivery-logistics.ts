...
@Component({
  selector: 'app-delivery-logistics',
  standalone: true,
    ...
    <div class="p-4 md:p-8 space-y-8 max-w-4xl mx-auto">
      ...
      <div class="card space-y-6 border-none ring-1 ring-slate-100 dark:ring-white/5">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-bold text-[#1A1A1A] dark:text-white">Delivery Configuration</h3>
        </div>

        <!-- Shipping Ranges -->
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <div class="text-xs font-bold text-slate-500 block mb-2 uppercase tracking-wide">Shipping Ranges</div>
            <button (click)="addShippingRange()" class="text-xs font-bold text-[#FFC107] hover:text-[#FFA000] flex items-center gap-1">
          ...
          <div class="hidden sm:grid grid-cols-5 gap-4 mb-2 px-4 shadow-sm py-2 bg-slate-50 dark:bg-white/5 rounded-lg">
            <div class="text-[10px] font-bold text-slate-500 uppercase">Min Order Amount (₹)</div>
            <div class="text-[10px] font-bold text-slate-500 uppercase">Max Order Amount (₹)</div>
            <div class="text-[10px] font-bold text-slate-500 uppercase">Type</div>
            <div class="text-[10px] font-bold text-slate-500 uppercase">Charge</div>
            <div></div>
          </div>
          ...
          @for (range of shippingRanges; track $index) {
            <div class="grid grid-cols-1 sm:grid-cols-5 gap-4 items-center bg-white dark:bg-[#1e293b]/50 p-4 rounded-xl border border-slate-100 dark:border-white/5 shadow-sm sm:shadow-none sm:p-0 sm:bg-transparent sm:border-none sm:dark:bg-transparent sm:dark:border-none">
              <div class="space-y-1 sm:space-y-0 text-left">
                <span class="sm:hidden text-[10px] font-bold text-slate-400 uppercase">Min Order (₹)</span>
                <input type="number" [(ngModel)]="range.min_amount" placeholder="Min Amount" class="w-full bg-[#F8F9FA] dark:bg-[#0F172A] border border-slate-100 dark:border-white/5 rounded-lg px-4 py-2 text-sm outline-none focus:ring-1 focus:ring-[#FFC107] dark:text-white">
              </div>
              <div class="space-y-1 sm:space-y-0 text-left">
                <span class="sm:hidden text-[10px] font-bold text-slate-400 uppercase">Max Order (₹)</span>
                <input type="number" [(ngModel)]="range.max_amount" placeholder="Max Amount" class="w-full bg-[#F8F9FA] dark:bg-[#0F172A] border border-slate-100 dark:border-white/5 rounded-lg px-4 py-2 text-sm outline-none focus:ring-1 focus:ring-[#FFC107] dark:text-white">
              </div>
              <div class="space-y-1 sm:space-y-0 text-left">
              ...
              <div class="space-y-1 sm:space-y-0 text-left">
                <span class="sm:hidden text-[10px] font-bold text-slate-400 uppercase">Charge ({{ range.calculation_type === 'percentage' ? '%' : '₹' }})</span>
                <div class="relative">
                  <span *ngIf="range.calculation_type === 'fixed'" class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">₹</span>
                  <input type="number" [(ngModel)]="range.price" 
              ...
              <div class="flex justify-end pt-2 sm:pt-0">
                <button (click)="removeShippingRange($index)" class="text-red-500 hover:text-red-700 w-10 h-10 flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors">
          ...
          @if (shippingRanges.length === 0) {
            <div class="text-center py-8 text-slate-500 text-sm border border-dashed border-slate-200 dark:border-white/10 rounded-xl">
              No shipping ranges configured. Click "Add Range" to create one.
            </div>
          }
        </div>
      </div>
...
export class DeliveryLogistics implements OnInit {
  ...
  save() {
    // Basic validation
    for (const range of this.shippingRanges) {
      if (Number(range.min_amount) >= Number(range.max_amount)) {
        this.toast.error('Min amount must be less than Max amount for all ranges');
        return;
      }
      if (range.price < 0) {
    ...
    this.toast.success('Logistics settings saved successfully');
  }
  ...
  addShippingRange() {
    const min = this.shippingRanges.length > 0 ? Math.max(...this.shippingRanges.map(r => r.max_amount)) + 1 : 0;
    this.shippingRanges.push({ min_amount: min, max_amount: min + 100, price: 10, calculation_type: 'fixed' });
  }
